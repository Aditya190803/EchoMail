 ...
 */
function extractFileName(url: string, contentDisposition?: string | null, recipientName?: string, contentType?: string): string {
  ...
  if (contentType) {
    if (contentType.includes('word') || contentType.includes('docx')) extension = '.docx'
...
export async function fetchFileFromUrl(
  ...
  // Determine filename
  let fileName = customFileName || extractFileName(url, contentDisposition, recipientName, contentType)
  
  console.log(`✅ Fetched file: ${fileName} (${(buffer.length / 1024).toFixed(1)} KB, ${contentType})`)
...
export function isAttachmentUrl(url: string): boolean {
  ...
  const extensions = ['.pdf', '.doc', '.docx', '.ppt', '.pptx', '.xls', '.json', '.xlsx', '.zip', '.jpg', '.png', '.jpeg']
  ...
  // OneDrive
  if (trimmed.includes('1drv.ms') || trimmed.includes('onedrive') || trimmed.includes('sharepoint')) return true
  
  // Dropbox
  if (trimmed.includes('dropbox.com')) return true
  
  // Check for common patterns
  if (trimmed.includes('pdf') || trimmed.includes('certificate') || trimmed.includes('document') || trimmed.includes('attachment')) return true
...
export function detectAttachmentColumn(headers: string[]): string | null {
  const keywords = [
    'pdf', 'certificate', 'attachment', 'document', 'file', 
    'link', 'url', 'drive', 'onedrive', 'dropbox',
    'doc', 'docx', 'ppt', 'pptx'
  ]
  ...
  for (const header of headers) {
    ...
    // Exact matches first
    if (lowerHeader === 'pdf' || lowerHeader === 'attachment' || lowerHeader === 'file' || lowerHeader === 'link') {
      return header
    }
 ...
 */
export async function validateAttachmentUrl(url: string): Promise<{ valid: boolean; error?: string }> {
  try {
    const directUrl = getDirectDownloadUrl(url)
