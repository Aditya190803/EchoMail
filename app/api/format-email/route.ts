...
import { formatEmailHTML, convertEmojiImagesToText } from '@/lib/email-formatter';
  ...
  try {
    ...
    return NextResponse.json({
      ...
      analysis: {
        ...
        hasEmojiImages: originalContent.includes('<img') && originalContent.includes('emoji'),
        emojiImagesRemoved: !contentWithTextEmojis.includes('<img') || !contentWithTextEmojis.includes('emoji'),
        isMjmlCompiled: formattedHTML.includes('mj-') || formattedHTML.length > originalContent.length * 2
      }
    });
  } catch (error) {
