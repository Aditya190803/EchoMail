...
const getAttachmentUrl = (a: { fileUrl?: string; appwrite_file_id?: string }) => {
  if (a.appwrite_file_id) return `/api/appwrite/attachments/${a.appwrite_file_id}`;
  if (a.fileUrl) {
    const m = a.fileUrl.match(/\/files\/([^\/]+)\//);
    if (m?.[1]) return `/api/appwrite/attachments/${m[1]}`;
  }
  return a.fileUrl || "#";
};
...
function getRecipientsArray(r: any): string[] {
  ...
  if (typeof r === "string") return r.split(",").map((e) => e.trim()).filter(Boolean);
  return [];
}
...
export default function DashboardPage() {
  ...
  /* load preview html */
  useEffect(() => {
    if (!selectedCampaign?.content) { setPreviewHtml(""); return; }
    setIsLoadingPreview(true);
    getEmailPreview(selectedCampaign.content)
      .then((h) => setPreviewHtml(createGmailPreviewWrapper(h)))
      .catch(() => setPreviewHtml(createGmailPreviewWrapper(selectedCampaign!.content!)))
  ...
  useEffect(() => {
    if (!session?.user?.email) return;
    fetchCampaigns();
    const unsub = campaignsService.subscribeToUserCampaigns(session.user.email, () => fetchCampaigns());
  ...
  const duplicateCampaign = (c: EmailCampaign) => {
    ...
    sessionStorage.setItem("duplicateCampaign", JSON.stringify({
  ...
  const exportCampaignResults = (c: EmailCampaign) => {
    ...
    const rows: string[][] = c.send_results?.length
      ? c.send_results.map((r: any) => [r.email || "", r.success ? "Sent" : "Failed", r.error || "", r.timestamp || c.created_at || ""])
      : getRecipientsArray(c.recipients).map((e) => [e, "Unknown", "", c.created_at || ""]);
    ...
    const a = Object.assign(document.createElement("a"), { href: url, download: `campaign_results_${c.subject.replace(/[^a-z0-9]/gi, "_").toLowerCase()}.csv` });
  ...
  /* guards */
  if (status === "loading" || !isMounted || isLoading) return <DashboardSkeleton />;
  ...
  return (
    <>
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          if (!open) { setSelectedCampaign(null); setRecipientSearch(""); setFilterStatus("all"); }
        }}
      >
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="flex items-center justify-between">
              ...
              {selectedCampaign && (
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => exportCampaignResults(selectedCampaign)}>
                    <Download className="h-4 w-4 mr-2" />Export
                  </Button>
                  <Button variant="outline" size="sm" disabled={isDuplicating} onClick={() => { duplicateCampaign(selectedCampaign); setSelectedCampaign(null); }}>
          ...
          {selectedCampaign && (
            <div className="flex-1 overflow-y-auto space-y-5 pr-1">
              {/* campaign stat strip */}
              <div className="border-l-4 border-primary rounded-lg bg-muted/20 p-4 space-y-3">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <p className="font-semibold">{selectedCampaign.subject}</p>
                    <p className="text-xs text-muted-foreground mt-0.5">Sent {formatDate(selectedCampaign.created_at)}</p>
                  </div>
                  <Badge variant={selectedCampaign.status === "completed" ? "success" : selectedCampaign.status === "sending" ? "warning" : "destructive"} className="capitalize shrink-0">
                    {selectedCampaign.status}
                  </Badge>
                </div>
                <div className="grid grid-cols-4 gap-3">
                  {[
                    { label: "Total", value: getRecipientsArray(selectedCampaign.recipients).length, colour: "text-foreground" },
                    { label: "Sent", value: selectedCampaign.sent, colour: "text-emerald-600" },
                    { label: "Failed", value: selectedCampaign.failed, colour: "text-red-500" },
                    { label: "Files", value: selectedCampaign.attachments?.length || 0, colour: "text-muted-foreground" },
                  ].map((s) => (
              ...
              <div className="space-y-2">
                <h4 className="font-medium text-sm flex items-center gap-2">
                  <Eye className="h-4 w-4 text-muted-foreground" />Email Preview
                </h4>
                <div className="border rounded-lg overflow-hidden">
                  ...
                  {isLoadingPreview ? (
                    ...
                    <iframe srcDoc={previewHtml} className="w-full h-[280px] border-0 bg-white" title="Email preview" sandbox="allow-same-origin" />
                  ) : (
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {selectedCampaign.attachments!.map((att, i) => (
                      <div key={i} className="flex items-center gap-2 bg-muted/30 border rounded-lg p-3">
                        <FileText className="h-4 w-4 text-primary shrink-0" />
                        <div className="min-w-0 flex-1">
                          <p className="text-sm font-medium truncate">{att.fileName}</p>
                          <p className="text-xs text-muted-foreground">{(att.fileSize / 1024 / 1024).toFixed(2)} MB</p>
                        </div>
              ...
              <div className="space-y-3">
                  ...
                  {selectedCampaign.send_results && (
                    <div className="flex gap-1">
                      {(["all", "success", "failed"] as const).map((f) => (
                        <Button key={f} variant={filterStatus === f ? (f === "success" ? "default" : f === "failed" ? "destructive" : "default") : "outline"} size="sm" className="h-8 capitalize text-xs" onClick={() => setFilterStatus(f)}>
                          {f === "success" ? <CheckCircle className="h-3 w-3 mr-1" /> : f === "failed" ? <XCircle className="h-3 w-3 mr-1" /> : null}
                          {f}
                        </Button>
                      ))}
                    </div>
                ...
                <div className="border rounded-lg max-h-56 overflow-y-auto divide-y">
                  {selectedCampaign.send_results?.length ? (
                    selectedCampaign.send_results
                      .filter((r: any) => {
                        const match = (r.email || "").toLowerCase().includes(recipientSearch.toLowerCase());
                        const filt = filterStatus === "all" || (filterStatus === "success" && r.status === "success") || (filterStatus === "failed" && r.status !== "success");
                        return match && filt;
                      })
                      .map((r: any, i: number) => (
                        <div key={i} className="flex items-center gap-3 px-4 py-2.5 hover:bg-muted/30 text-sm">
                          {r.status === "success"
                            ? <CheckCircle className="h-3.5 w-3.5 text-emerald-500 shrink-0" />
                            : <XCircle className="h-3.5 w-3.5 text-red-500 shrink-0" />}
                          <span className="flex-1 truncate">{r.email}</span>
                          {r.error && <span className="text-xs text-red-500 truncate max-w-[160px]">{r.error}</span>}
                          <Badge variant={r.status === "success" ? "success" : "destructive"} className="text-[10px] shrink-0">
                    ...
                    <div className="p-3 grid grid-cols-2 gap-2">
                      {getRecipientsArray(selectedCampaign.recipients)
                        ...
                        .map((e, i) => (
                          <div key={i} className="text-sm bg-muted/30 rounded-lg px-3 py-1.5 truncate border">{e}</div>
                        ))}
                    </div>
                  )}
                </div>
