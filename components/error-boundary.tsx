          ...
          {isChunkError ? (
            <div className="space-y-2">
              <p className="text-sm text-gray-600">
                A loading error occurred. This usually happens when the application updates while you're using it.
              </p>
              <p className="text-sm text-gray-600">
  ...
  React.useEffect(() => {
    const handleError = (event: ErrorEvent) => {
      if (event.error?.name === 'ChunkLoadError' || event.message.includes('Loading chunk')) {
        console.log('ChunkLoadError detected in hook, reloading...')
        window.location.reload()
      }
    }
    ...
    const handleUnhandledRejection = (event: PromiseRejectionEvent) => {
      if (event.reason?.name === 'ChunkLoadError' || event.reason?.message?.includes('Loading chunk')) {
        console.log('ChunkLoadError promise rejection detected, reloading...')
        window.location.reload()
      }
    }
