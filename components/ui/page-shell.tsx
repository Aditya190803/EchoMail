...
export function StatCard({ label, value, sub, icon, trend, accentClass = "border-primary", children }: StatCardProps) {
  const trendColour = trend?.direction === "up" ? "text-emerald-500" : trend?.direction === "down" ? "text-destructive" : "text-muted-foreground";
  const trendArrow = trend?.direction === "up" ? "↑" : trend?.direction === "down" ? "↓" : "→";

  return (
    <div className={`rounded-xl border-l-4 bg-card p-5 hover:shadow-md transition-all duration-200 ${accentClass}`}>
