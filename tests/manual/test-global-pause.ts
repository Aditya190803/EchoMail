...
function triggerGlobalPause(reason: string, durationMs: number = 300000) {
  console.log(
    `🚨 TRIGGERING GLOBAL EMAIL PAUSE: ${reason} for ${durationMs / 1000}s`,
  );
...
function checkGlobalPause(): boolean {
  ...
  if (now >= pauseEndTime) {
    console.log(
      `✅ GLOBAL EMAIL PAUSE LIFTED after ${(now - global.emailRateLimitState.pauseStartTime) / 1000}s`,
    );
  ...
  console.log(
    `⏸️ GLOBAL EMAIL PAUSE ACTIVE: ${global.emailRateLimitState.pauseReason} - ${Math.ceil(remainingTime / 1000)}s remaining`,
  );
  return true;
}
